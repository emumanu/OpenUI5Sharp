	...
	public partial class sap
	{
		public static partial class ui
		{
			public static partial class model
			{
				...
				{
					/// <summary>
					/// A collection of methods which help to consume <a href="http://docs.oasis-open.org/odata/odata/v4.0/odata-v4.0-part3-csdl.html"> OData V4 annotations</a> in XML template views. Every context argument must belong to a <code>sap.ui.model.odata.ODataMetaModel</code> instance.
					/// 
					...
					public static partial class AnnotationHelper
					{
						...
						/// </pre>
						/// </summary>
						/// <param name="vParts">array of parts</param>
						/// <param name="fnRootFormatter">root formatter function; default: <code>Array.prototype.join(., " ")</code> in case of multiple parts, just like {@link sap.ui.model.CompositeBinding#getExternalValue getExternalValue}</param>
						/// <returns>constant value or binding info object for a property as expected by {@link sap.ui.base.ManagedObject#applySettings applySettings}</returns>
						public extern static object createPropertySetting(object[] vParts, object fnRootFormatter);
						...
						/// <param name="vParts">array of parts</param>
						/// <returns>constant value or binding info object for a property as expected by {@link sap.ui.base.ManagedObject#applySettings applySettings}</returns>
						public extern static object createPropertySetting(object[] vParts);
						...
						/// </pre>
						/// </summary>
						/// <param name="oInterface">the callback interface related to the current formatter call</param>
						/// <param name="vRawValue">the raw value from the meta model, which is embedded within an entity set or entity type: <ul> <li>if this function is used as formatter the value is provided by the framework</li> <li>if this function is called directly, provide the parameter only if it is already calculated</li> <li>if the parameter is omitted, it is calculated automatically through <code>oInterface.getObject("")</code></li> </ul></param>
						/// <returns>the resulting string value to write into the processed XML</returns>
						public extern static string format(Union<sap.ui.core.util.XMLPreprocessor.IContext, sap.ui.model.Context> oInterface, object vRawValue);
						...
						/// <param name="oInterface">the callback interface related to the current formatter call</param>
						/// <returns>the resulting string value to write into the processed XML</returns>
						public extern static string format(Union<sap.ui.core.util.XMLPreprocessor.IContext, sap.ui.model.Context> oInterface);
						...
						/// </summary>
						/// <param name="oInterface">the callback interface related to the current formatter call</param>
						/// <param name="vRawValue">the raw value from the meta model, e.g. <code>{AnnotationPath : "ToSupplier/@com.sap.vocabularies.Communication.v1.Address"}</code> or <code> {AnnotationPath : "@com.sap.vocabularies.UI.v1.FieldGroup#Dimensions"}</code>; embedded within an entity set or entity type; <ul> <li>if this function is used as formatter the value is provided by the framework</li> <li>if this function is called directly, provide the parameter only if it is already calculated</li> <li>if the parameter is omitted, it is calculated automatically through <code>oInterface.getObject("")</code></li> </ul></param>
						/// <returns>the resulting string value to write into the processed XML, e.g. "{ToSupplier}" or "{}" (in case no navigation is needed); returns "" in case the navigation path cannot be determined (this is treated as falsy in <code>template:if</code> statements!)</returns>
						public extern static string getNavigationPath(Union<sap.ui.core.util.XMLPreprocessor.IContext, sap.ui.model.Context> oInterface, object vRawValue);
						...
						/// <param name="oInterface">the callback interface related to the current formatter call</param>
						/// <returns>the resulting string value to write into the processed XML, e.g. "{ToSupplier}" or "{}" (in case no navigation is needed); returns "" in case the navigation path cannot be determined (this is treated as falsy in <code>template:if</code> statements!)</returns>
						public extern static string getNavigationPath(Union<sap.ui.core.util.XMLPreprocessor.IContext, sap.ui.model.Context> oInterface);
						...
						/// </pre>
						/// </summary>
						/// <param name="oContext">a context which must point to a simple string or to an annotation (or annotation property) of type <code>Edm.AnnotationPath</code>, <code>Edm.NavigationPropertyPath</code>, <code>Edm.Path</code>, or <code>Edm.PropertyPath</code> embedded within an entity set or entity type; the context's model must be an {@link sap.ui.model.odata.ODataMetaModel}</param>
						/// <returns>the path to the entity set, or <code>undefined</code> if no such set is found. In this case, a warning is logged to the console.</returns>
						public extern static string gotoEntitySet(sap.ui.model.Context oContext);
						...
						/// </pre>
						/// </summary>
						/// <param name="oContext">a context which must point to the qualified name of an entity type; the context's model must be an {@link sap.ui.model.odata.ODataMetaModel}</param>
						/// <returns>the path to the entity type with the given qualified name, or <code>undefined</code> if no such type is found. In this case, a warning is logged to the console.</returns>
						public extern static string gotoEntityType(sap.ui.model.Context oContext);
						...
						/// </pre>
						/// </summary>
						/// <param name="oContext">a context which must point to an object with a <code>String</code> property, which holds the qualified name of the function import; the context's model must be an {@link sap.ui.model.odata.ODataMetaModel}</param>
						/// <returns>the path to the function import with the given qualified name, or <code>undefined</code> if no function import is found. In this case, a warning is logged to the console.</returns>
						public extern static string gotoFunctionImport(sap.ui.model.Context oContext);
						...
						/// <param name="oInterface">the callback interface related to the current formatter call</param>
						/// <param name="vRawValue">the raw value from the meta model, e.g. <code>{AnnotationPath : "ToSupplier/@com.sap.vocabularies.Communication.v1.Address"}</code> or <code> {AnnotationPath : "@com.sap.vocabularies.UI.v1.FieldGroup#Dimensions"}</code>; embedded within an entity set or entity type; <ul> <li>if this function is used as formatter the value is provided by the framework</li> <li>if this function is called directly, provide the parameter only if it is already calculated</li> <li>if the parameter is omitted, it is calculated automatically through <code>oInterface.getObject("")</code></li> </ul></param>
						/// <returns><code>"true"</code> if the navigation path ends with an association end with multiplicity "*", <code>""</code> in case the navigation path cannot be determined, <code>"false"</code> otherwise (the latter are both treated as falsy in <code>template:if</code> statements!)</returns>
						public extern static string isMultiple(Union<sap.ui.core.util.XMLPreprocessor.IContext, sap.ui.model.Context> oInterface, object vRawValue);
						...
						/// <param name="oInterface">the callback interface related to the current formatter call</param>
						/// <returns><code>"true"</code> if the navigation path ends with an association end with multiplicity "*", <code>""</code> in case the navigation path cannot be determined, <code>"false"</code> otherwise (the latter are both treated as falsy in <code>template:if</code> statements!)</returns>
						public extern static string isMultiple(Union<sap.ui.core.util.XMLPreprocessor.IContext, sap.ui.model.Context> oInterface);
						...
						/// <returns>the path to the target, or <code>undefined</code> in case the path cannot be resolved. In this case, a warning is logged to the console.</returns>
						public extern static string resolvePath(sap.ui.model.Context oContext);
						...
						/// <param name="vRawValue">the raw value from the meta model, which is embedded within an entity set or entity type: <ul> <li>if this function is used as formatter the value is provided by the framework</li> <li>if this function is called directly, provide the parameter only if it is already calculated</li> <li>if the parameter is omitted, it is calculated automatically through <code>oInterface.getObject("")</code></li> </ul></param>
						/// <returns>the resulting string value to write into the processed XML</returns>
						public extern static string simplePath(Union<sap.ui.core.util.XMLPreprocessor.IContext, sap.ui.model.Context> oInterface, object vRawValue);
						...
						/// <param name="oInterface">the callback interface related to the current formatter call</param>
						/// <returns>the resulting string value to write into the processed XML</returns>
						public extern static string simplePath(Union<sap.ui.core.util.XMLPreprocessor.IContext, sap.ui.model.Context> oInterface);
